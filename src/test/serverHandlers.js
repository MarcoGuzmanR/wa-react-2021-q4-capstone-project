  ...
  "results": [
    {
      ...
      "url": null,
      "type": "product",
      "href": "https://wizeline-academy.cdn.prismic.io/api/v2/documents/search?ref=YXc6vhIAACsAyYIc&q=%5B%5B%3Ad+%3D+at%28document.id%2C+%22YWMHuxIAAC8Azxdw%22%29+%5D%5D",
      "tags": [
        "Living Room",
        "Armchair",
        "Featured"
...
const handlers = [
  rest.get(`${API_BASE_URL}/documents/search?ref=YXc6vhIAACsAyYIc&q=%5B%5B%3Ad+%3D+at%28document.id%2C+%22${productId}%22%29+%5D%5D`, (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json(mockedProduct),
    )
  }),

  rest.get(`${API_BASE_URL}/documents/search?ref=YXc6vhIAACsAyYIc&q=${encodeURIComponent(`[[at(document.type, "category")]]`)}`, (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json(mockedCategories),
    )
  }),
